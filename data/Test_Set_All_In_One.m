...
for k=Reduced_hz/process_hz*num_time_step:size(ReducedDataCollision,1)
    ...
    for time_step=1:num_time_step
        for joint_data=1:6
            TestProcessNocutData(TestProcessNocutDataIdx,(num_data_type-1)*num_time_step*(joint_data-1)+(num_data_type-1)*(time_step-1)+1) = 2*(ReducedDataCollision(k-Reduced_hz/process_hz*(time_step-1),1+joint_data) - ReducedDataCollision(k-Reduced_hz/process_hz*(time_step-1),31+joint_data) - MinCurrentDyna(joint_data)) / (MaxCurrentDyna(joint_data)-MinCurrentDyna(joint_data)) -1; % dyna_torque - current_torque
            TestProcessNocutData(TestProcessNocutDataIdx,(num_data_type-1)*num_time_step*(joint_data-1)+(num_data_type-1)*(time_step-1)+2) = 2*(ReducedDataCollision(k-Reduced_hz/process_hz*(time_step-1),13+joint_data) - MinTrainingData(1,13+joint_data)) / (MaxTrainingData(1,13+joint_data) - MinTrainingData(1,13+joint_data)) -1; % qdot
            TestProcessNocutData(TestProcessNocutDataIdx,(num_data_type-1)*num_time_step*(joint_data-1)+(num_data_type-1)*(time_step-1)+3) = 2*(ReducedDataCollision(k-Reduced_hz/process_hz*(time_step-1),19+joint_data) - ReducedDataCollision(k-Reduced_hz/process_hz*(time_step-1),7+joint_data) - MinDeltaQdQ(joint_data)) / (MaxDeltaQdQ(joint_data)-MinDeltaQdQ(joint_data)) -1; % dyna_torque - current_torque
            TestProcessNocutData(TestProcessNocutDataIdx,(num_data_type-1)*num_time_step*(joint_data-1)+(num_data_type-1)*(time_step-1)+4) = 2*(ReducedDataCollision(k-Reduced_hz/process_hz*(time_step-1),25+joint_data) - ReducedDataCollision(k-Reduced_hz/process_hz*(time_step-1),13+joint_data) - MinDeltaQdotdQdot(joint_data)) / (MaxDeltaQdotdQdot(joint_data)-MinDeltaQdotdQdot(joint_data)) -1; % dyna_torque - current_torque
            TestProcessNocutData(TestProcessNocutDataIdx,(num_data_type-1)*num_time_step*(joint_data-1)+(num_data_type-1)*(time_step-1)+5) = 2*(ReducedDataCollision(k-Reduced_hz/process_hz*(time_step-1),37+joint_data) - ReducedDataCollision(k-Reduced_hz/process_hz*(time_step-1),7+joint_data) - MinDeltaThetaQ(joint_data)) / (MaxDeltaThetaQ(joint_data)-MinDeltaThetaQ(joint_data)) -1; % qdot error
        end
        TestProcessNocutData(TestProcessNocutDataIdx,(num_data_type-1)*num_time_step*6+time_step) = (norm(ReducedDataCollision(k-Reduced_hz/process_hz*(time_step-1),62:64))-MinAcc) / (MaxAcc-MinAcc); % end effector acceleration
    end
    TestProcessNocutDataIdx = TestProcessNocutDataIdx +1;
end

cd ..
csvwrite('TestingDataCollision.csv', TestProcessNocutData(1:TestProcessNocutDataIdx-1,:));
...
for k=Reduced_hz/process_hz*num_time_step:size(ReducedTestDataFree,1)
    ...
    for time_step=1:num_time_step
        for joint_data=1:6
            TestProcessFreeData(TestProcessFreeDataIdx,(num_data_type-1)*num_time_step*(joint_data-1)+(num_data_type-1)*(time_step-1)+1) = 2*(ReducedTestDataFree(k-Reduced_hz/process_hz*(time_step-1),1+joint_data) - ReducedTestDataFree(k-Reduced_hz/process_hz*(time_step-1),31+joint_data) - MinCurrentDyna(joint_data)) / (MaxCurrentDyna(joint_data)-MinCurrentDyna(joint_data)) -1; % dyna_torque - current_torque
            TestProcessFreeData(TestProcessFreeDataIdx,(num_data_type-1)*num_time_step*(joint_data-1)+(num_data_type-1)*(time_step-1)+2) = 2*(ReducedTestDataFree(k-Reduced_hz/process_hz*(time_step-1),13+joint_data) - MinTrainingData(1,13+joint_data)) / (MaxTrainingData(1,13+joint_data) - MinTrainingData(1,13+joint_data)) -1; % qdot
            TestProcessFreeData(TestProcessFreeDataIdx,(num_data_type-1)*num_time_step*(joint_data-1)+(num_data_type-1)*(time_step-1)+3) = 2*(ReducedTestDataFree(k-Reduced_hz/process_hz*(time_step-1),19+joint_data) - ReducedTestDataFree(k-Reduced_hz/process_hz*(time_step-1),7+joint_data) - MinDeltaQdQ(joint_data)) / (MaxDeltaQdQ(joint_data)-MinDeltaQdQ(joint_data)) -1; % dyna_torque - current_torque
            TestProcessFreeData(TestProcessFreeDataIdx,(num_data_type-1)*num_time_step*(joint_data-1)+(num_data_type-1)*(time_step-1)+4) = 2*(ReducedTestDataFree(k-Reduced_hz/process_hz*(time_step-1),25+joint_data) - ReducedTestDataFree(k-Reduced_hz/process_hz*(time_step-1),13+joint_data) - MinDeltaQdotdQdot(joint_data)) / (MaxDeltaQdotdQdot(joint_data)-MinDeltaQdotdQdot(joint_data)) -1; % dyna_torque - current_torque
            TestProcessFreeData(TestProcessFreeDataIdx,(num_data_type-1)*num_time_step*(joint_data-1)+(num_data_type-1)*(time_step-1)+5) = 2*(ReducedTestDataFree(k-Reduced_hz/process_hz*(time_step-1),37+joint_data) - ReducedTestDataFree(k-Reduced_hz/process_hz*(time_step-1),7+joint_data) - MinDeltaThetaQ(joint_data)) / (MaxDeltaThetaQ(joint_data)-MinDeltaThetaQ(joint_data)) -1; % qdot error
        end
        TestProcessFreeData(TestProcessFreeDataIdx,(num_data_type-1)*num_time_step*6+time_step) = (norm(ReducedTestDataFree(k-Reduced_hz/process_hz*(time_step-1),62:64))-MinAcc) / (MaxAcc-MinAcc); % end effector acceleration
    end
