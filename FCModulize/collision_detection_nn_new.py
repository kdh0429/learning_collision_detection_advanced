        ...
        with tf.variable_scope(self.name, reuse = tf.AUTO_REUSE):
            ...
            for i in range(6):
                with tf.variable_scope("Joint"+str(i)+"Net"):
                    ...
                    W2 = tf.get_variable("W2", shape=[self.hidden_neurons, math.ceil(self.hidden_neurons/2)], initializer=tf.contrib.layers.xavier_initializer(), regularizer=tf.contrib.layers.l2_regularizer(scale=regul_factor))
                    b2 = tf.Variable(tf.random_normal([math.ceil(self.hidden_neurons/2)]))
                    ...
                    W3 = tf.get_variable("W3", shape=[math.ceil(self.hidden_neurons/2), math.ceil(self.hidden_neurons/4)], initializer=tf.contrib.layers.xavier_initializer(), regularizer=tf.contrib.layers.l2_regularizer(scale=regul_factor))
                    b3 = tf.Variable(tf.random_normal([math.ceil(self.hidden_neurons/4)]))
                    ...
                    W4 = tf.get_variable("W4", shape=[math.ceil(self.hidden_neurons/4), 1], initializer=tf.contrib.layers.xavier_initializer(), regularizer=tf.contrib.layers.l2_regularizer(scale=regul_factor))
...
TrainData = tf.data.TFRecordDataset(["../data/TrainingDataNewCollision.tfrecord"])
...
ValidationData = pd.read_csv('../data/ValidationDataNew.csv').as_matrix().astype('float64')
...
# Test Evaluation
TestData = pd.read_csv('../data/TestingDataNew.csv').as_matrix().astype('float64')
